...
module.exports = {
  ...
  createSubService: async function({id, subServiceData}, req) {
  	...
  	if (subServiceData.subServiceName == null || subServiceData.subServiceName == "") {
  	...
  	if (subServiceData.price == null || subServiceData.subServiceName == "") {
  		errors.push({message: "Please Insert SubService Price"});
  	}
  	if (!id || id.toString == "" || !service) {
  		errors.push({message: "Can't Insert SubService without its Service Category"});
  	}
  	if (errors.length > 0) {
      const error = new Error("Invalid input.");
      error.data = errors;
    ...
    const data = await SubService.findOne({ _id: datas._id }).populate("serviceId");
    return { 
    	...data._doc,
  ...
  updateSubService: async function({id, subServiceInput}, req) {
    ...
    const data = await SubService.findById(updatedSubService._id).populate("serviceId");
    return {
    	...data._doc,
    	_id: data._id.toString(),
    	createdAt: data.createdAt.toISOString(),
  ...
  createUser: async function({ token, expiresIn, serviceId, subServiceId, userInput }, req) 
  	{
		...
		const existingUser = await User.findOne({ $or: [ {email: userInput.email}, {telephone: userInput.telephone} ] });
		if (existingUser) {
		...
		if (userInput.userType === "Client") {
			const user = new User({
				...
				address: {province: userInput.address.province, district: userInput.address.district, sector: userInput.address.sector}
			});
		...
		if (userInput.userType === "Worker") {
			...
			const subService = await SubService.find({ _id: { $in: subServiceId } });
			if (validator.isEmpty(serviceId)){
				errors.push({ message: "Please Select any Service" });
			} 
			if(subServiceId.length <= 0) {
				errors.push({ message: "Please Select any SubService in that Service" });
	    	}
	    	if (!service) {
	    	...
	    	if (!subService) {
	    		const error = new Error("That SubService no longer available in That Service!");
	    		throw error;
	    	}
	    	if (validator.isEmpty(userInput.address.province) || 
			...
			const createdUsers = await user.save();
		 	const createdUser = await User.findOne({ _id: createdUsers._id }).populate("serviceId").populate("subServiceId");
		 	token = jwt.sign(
				{
					userId: createdUser._id.toString(),
					email: createdUser.email
	...
	updatedUser: async function({id, serviceId, subServiceId, userInput}, req) {
		...
		if (user.userType === "Client") {
			...
			user.address = {province: userInput.province, district: userInput.district, sector: userInput.sector};
	...
	createOrders: async function({clientId, orderInput}, req) {
		...
		if (new Date(orderInput.duration) < new Date()) {
			const error = new Error("Your Order's Duration is Less Than ToDay!");
	    	throw error;
		}
		if (!user) {
		...
		if (!service && !subService) {
			const error = new Error("That Service and its SubService are not Found!");
	    	throw error;
		}
	...
	viewOrders: async function({userId, yourId}, req) {
		...
		// }
		if (yourId) {
			const orders = await Order.find({ clientId: yourId })
				.sort({ createdAt: -1 })
				.populate("clientId")
				.populate("serviceId")
				.populate("subServiceId");
			const totalOrder = await Order.find({ clientId: yourId }).countDocuments();
			const totalOrders = await Order.find().countDocuments();
			return {
				orders: orders.map(order => {
					return {
		...
		const orders = await Order.find()
			...
			.populate("serviceId")
			.populate("subServiceId");
		const totalOrder = await Order.find({ clientId: userId }).countDocuments();
		const totalOrders = await Order.find().countDocuments();
		return {
			orders: orders.map(order => {
				return {
	...
	updateOrder: async function({clientId, orderId, orderInput}, req) {
		...
		if (order.clientId.toString() !== clientId.toString()) {
			const error = new Error('You are not allowed to Updating this Order.');
	        error.code = 401;
	        throw error;
		}
	...
	viewNewOrders: async function({userId}, req) {
		...
		if (user.userType === "Worker") {
			const newOrders = await Order.find({ $and: [
				 	{ $or: [ 
				 		{ serviceId: user.serviceId }, { subServiceId: { $in: user.subServiceId } 
				 		} ] },
				 	 { status: 0 } 
				] })
				.sort({ createdAt: -1 })
				...
				.populate("serviceId")
				.populate("subServiceId");
			const totalNewOrders = await Order.find({ $and: [
				 	{ $or: [ 
				 		{ serviceId: user.serviceId }, { subServiceId: { $in: user.subServiceId } 
				 		} ] },
				 	 { status: 0 } 
	...
	createBid: async function({orderId, workerId, bidInput}, req) {
		...
		if (new Date(bidInput.duration) < new Date(order.duration)) {
			const error = new Error("Your Bid's duration is Less Than Order's duration.");
	        error.code = 401;
	        throw error;
		}
		...
		if (user.serviceId.toString() !== order.serviceId.toString()) {
			const error = new Error('You are Not Allowed to serve that Service!');
		    error.code = 401;
		    throw error;
		}
		const existingBids = await Bid.findOne({$and: [ {orderId: order._id }, { workerId: user._id } ]});
		if (existingBids) {
			const error = new Error('You are not Allowed to Bid more than once on one order!');
      		throw error;
		}
		const bids = new Bid({
			orderId: order,
	...
	viewOfferBids: async function({clientId, offerId}, req) {
		const user = await Order.find({ clientId: clientId });
		if (!user) {
			const error = new Error('A user could not be found.');
	        error.code = 401;
	        throw error;
		...
		user.map(item => console.log(item._id));
		const offerBid = await Bid.find({ orderId: user.map(item => item._id) })
				.sort({ createdAt: -1 })
				.populate({
					path: "orderId",
					model: "Order",
		...
		const totalBids = await Bid.find({ $and: [ {orderId: offerId}, {status: 1}] })
				.countDocuments()
		return {
	...
	workerBids: async function({workerId}, req) {
		const worker = await Bid.find({ workerId: workerId})
			.sort({createdAt: -1});
		const totalBids = await Bid.find({ workerId: workerId})
			.countDocuments();
		if (!worker) {
			const error = new Error('A user could not be found.');
	        error.code = 401;
	...
	updatedBid: async function({workerId, bidId, bidInput}, req) {
		...
		if (bid.status === 1) {
			const error = new Error('You are not allowed to Update Submitted Bid.');
	        error.code = 401;
	        throw error;
		}
	...
	createCart: async function({clientId, cartInputs}, req) {
		...
		if (cart) {
			const check = cart.orders.map((item, key) => key === 0 && item.subServiceId.toString() === subService._id.toString() ? true : null );
			if (check[0] === true) {
	...
	updateCart: async function({ cartId, cartInputs}, req) {
		...
		const check = await Cart.updateMany({orders:{
			$elemMatch:{
		...
		{
			$set: {"orders.$.price": cartInputs.price,
	...
	deleteCart: async function({clientId, cartId, serviceId, subServiceId}, req) {
		const user = await User.findOne({ _id: clientId });
		var cart = await Cart.findOne({ _id: cartId }); 
		if (!user) {
		...
		if (!service || !subService) {
			const error = new Error("That Service and its SubService are not Found in Your Cart!");
		    throw error;
		}
		if (!cart) {
			const error = new Error('Cart not Found');
	...
	cartOrder: async function({clientId, cartId}, req) {
		...
		var cart = await Cart.findOne({ $and: [ { clientId: user._id }, { _id: cartId } ] });
		if (!cart) {
			const error = new Error('Cart not Found.');
	        error.code = 401;
	        throw error;
		...
		if (savedOrders && cartDelete) {
			const savedOrder = await Order.find({ _id: { $in: savedOrders.map(item => item._id) } })
